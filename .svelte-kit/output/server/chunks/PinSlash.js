...
function EmojiPicker($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let emojiRows = [];
    const ROW_HEIGHT = 48;
    recentEmojiNames = (store_get($$store_subs ??= {}, "$settings", settings)?.recentEmojis ?? []).filter((name) => emojiShortCodes[name]).slice(0, MAX_RECENT);
    {
      if (search) {
    ...
    }
    {
      ...
      if (!search && recentEmojiNames.length > 0) {
        flattenedEmojis.push({
          type: "group",
          label: store_get($$store_subs ??= {}, "$i18n", i18n).t("Recently Used")
        });
        flattenedEmojis.push(...recentEmojiNames.map((emoji) => ({
          type: "emoji",
          name: emoji,
    ...
    function $$render_inner($$renderer3) {
      Dropdown($$renderer3, {
        ...
        $$slots: {
          default: true,
          content: ($$renderer4) => {
            $$renderer4.push(`<div slot="content"><div class="max-w-full w-72 border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-850 rounded-xl z-9999 shadow-lg dark:text-white"><div class="mb-0.5 px-3 pt-2 pb-1.5"><input type="text" class="w-full text-[13px] bg-transparent outline-hidden"${attr("placeholder", store_get($$store_subs ??= {}, "$i18n", i18n).t("Search all emojis"))}${attr("value", search)}/></div> <div class="w-full flex justify-start h-96 overflow-y-auto px-2.5 pb-2.5 text-[13px]">`);
              ...
              VirtualList($$renderer4, {
                ...
                $$slots: {
                  default: ($$renderer5, { item }) => {
                    $$renderer5.push(`<div class="w-full mb-2.5">`);
                    if (item.length === 1 && item[0].type === "group") {
                      $$renderer5.push("<!--[0-->");
                      $$renderer5.push(`<div class="text-xs font-normal -mb-1 text-gray-500 dark:text-gray-400">${escape_html(item[0].label)}</div>`);
                    } else {
                      $$renderer5.push("<!--[-1-->");
                      ...
                      for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
                        ...
                        Tooltip($$renderer5, {
                          ...
                          placement: "top",
                          children: ($$renderer6) => {
                            $$renderer6.push(`<button${attr_class(`p-1 rounded-lg cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700 transition ${selected === emojiItem.shortCodes[0] ? "bg-gray-200 dark:bg-gray-700" : ""}`)}><img${attr("src", `${stringify(WEBUI_BASE_URL)}/assets/emojis/${stringify(emojiItem.name.toLowerCase())}.svg`)}${attr("alt", emojiItem.name)} class="size-5" loading="lazy"/></button>`);
                          },
                          $$slots: { default: true }
                        });
                      }
    ...
    bind_props($$props, { onClose, onSubmit, side, align, user, selected });
  });
}
