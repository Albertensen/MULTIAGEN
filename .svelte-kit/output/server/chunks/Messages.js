import { o as getContext, f as fallback, k as escape_html, b as bind_props, e as ensure_array_like, a as attr, d as attr_class, c as store_get, u as unsubscribe_stores, t as stringify, j as slot, g as clsx, h as attr_style } from "./root.js";
import { v4 } from "uuid";
...
import { a as WEBUI_API_BASE_URL, J as replaceOutsideCode, x as removeAllDetails, K as formatMessageTimestampFull, w as sanitizeResponseContent, L as formatMessageTimestamp, W as WEBUI_BASE_URL, c as WEBUI_VERSION } from "./index3.js";
...
import { T as Textarea, E as EllipsisHorizontal } from "./EllipsisHorizontal.js";
...
import { C as Collapsible, a as ChevronUp } from "./Download.js";
...
function TagItem($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    if (tag) {
      ...
      if (disabled) {
        $$renderer2.push("<!--[0-->");
        $$renderer2.push(`<span class="flex items-center gap-1 px-1.5 py-[1px] rounded-full bg-gray-100/50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800 text-gray-600 dark:text-gray-300 text-xs font-normal"><span class="line-clamp-1">${escape_html(tag.name)}</span></span>`);
      } else {
        $$renderer2.push("<!--[-1-->");
        $$renderer2.push(`<button type="button" class="flex items-center gap-1 px-1.5 py-[1px] rounded-full bg-gray-100/50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800 text-gray-600 dark:text-gray-300 text-xs font-normal hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"><span class="line-clamp-1">${escape_html(tag.name)}</span> `);
...
function TagList($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let tags2 = fallback($$props["tags"], () => [], true);
    let disabled = fallback($$props["disabled"], false);
    $$renderer2.push(`<!--[-->`);
    const each_array = ensure_array_like(tags2);
    ...
    $$renderer2.push(`<!--]-->`);
    bind_props($$props, { tags: tags2, disabled });
  });
}
function Tags($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    tagNames = new Set((tags2 ?? []).map((tag) => tag.name.toLowerCase()));
    (suggestionTags ?? []).map((tag) => (typeof tag === "string" ? tag : tag?.name ?? "").trim()).filter((name) => name && !tagNames.has(name.toLowerCase()) && name.toLowerCase().includes(inputValue.trim().toLowerCase())).slice(0, 8);
    $$renderer2.push(`<div class="flex flex-wrap items-center gap-1 w-full">`);
    TagList($$renderer2, { tags: tags2, disabled });
    $$renderer2.push(`<!----> `);
    if (!disabled) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<div class="flex-1 min-w-24"><input${attr("value", inputValue)}${attr_class(`w-full ${tags2.length > 0 ? "px-0.5" : ""} text-xs bg-transparent outline-hidden placeholder:text-gray-400 dark:placeholder:text-gray-500`)}${attr("placeholder", store_get($$store_subs ??= {}, "$i18n", i18n).t("Add a tag..."))} role="combobox" aria-autocomplete="list"${attr("aria-expanded", suggestionsOpen)} autocomplete="off"/> `);
      {
        $$renderer2.push("<!--[-1-->");
      }
      $$renderer2.push(`<!--]--></div>`);
    ...
    bind_props($$props, { tags: tags2, suggestionTags, disabled });
  });
}
function RateComment($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    if (
      ...
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<div class="text-xs font-normal pt-1.5 -mb-0.5">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t('This response was generated by "{{model}}"', {
        model: message.selectedModelId
      }))}</div>`);
    } else {
      $$renderer2.push("<!--[-1-->");
    }
    $$renderer2.push(`<!--]--> <div class="my-2.5 rounded-xl px-4 py-3 border border-gray-100/30 dark:border-gray-850/30"${attr("id", `message-feedback-${stringify(message.id)}`)}><div class="flex justify-between items-center"><div class="text-sm font-normal">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("How would you rate this response?"))}</div> <button${attr("aria-label", store_get($$store_subs ??= {}, "$i18n", i18n).t("Close feedback"))}>`);
    XMark($$renderer2, { className: "size-4" });
    $$renderer2.push(`<!----></button></div> <div class="w-full overflow-x-auto"><div class="relative mx-auto min-w-[19rem] max-w-80"><div class="mt-1.5 grid grid-cols-10 gap-1 pb-2"><!--[-->`);
    const each_array = ensure_array_like(Array.from({ length: 10 }).map((_, i) => i + 1));
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      let rating = each_array[$$index];
      $$renderer2.push(`<button${attr("aria-label", store_get($$store_subs ??= {}, "$i18n", i18n).t("Rate {{rating}} out of 10", { rating }))}${attr_class(`aspect-square w-full text-sm border border-gray-100/30 dark:border-gray-850/30 hover:bg-gray-50 dark:hover:bg-gray-850 ${detailedRating === rating ? "bg-gray-100 dark:bg-gray-800" : ""} transition rounded-full disabled:cursor-not-allowed disabled:text-gray-500 disabled:bg-white dark:disabled:bg-gray-900`)}${attr("disabled", message?.annotation?.rating === -1 ? rating > 5 : rating < 6, true)}>${escape_html(rating)}</button>`);
    }
    ...
    if (reasons.length > 0) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<div class="text-sm mt-1.5 font-normal">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Why?"))}</div> <div class="flex flex-wrap gap-1.5 text-sm mt-1.5"><!--[-->`);
      const each_array_1 = ensure_array_like(reasons);
      for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
        let reason = each_array_1[$$index_1];
        $$renderer2.push(`<button${attr_class(`px-3 py-0.5 border border-gray-100/30 dark:border-gray-850/30 hover:bg-gray-50 dark:hover:bg-gray-850 ${selectedReason === reason ? "bg-gray-100 dark:bg-gray-800" : ""} transition rounded-xl`)}>`);
...
function Error($$renderer, $$props) {
  ...
  $$renderer.push(`<div class="my-1.5 flex w-full items-start gap-2 rounded-2xl bg-black/[0.03] px-3 py-2 text-gray-500 dark:bg-white/[0.04] dark:text-gray-400">`);
  Info($$renderer, {
    className: "mt-0.5 size-4 shrink-0 text-gray-400 dark:text-gray-500",
    strokeWidth: "1.8"
  });
  $$renderer.push(`<!----> <div class="min-w-0 break-words text-[0.8125rem] leading-5">${escape_html(message)}</div></div>`);
  ...
  $$renderer.component(($$renderer2) => {
    ...
    function $$render_inner($$renderer3) {
      Modal($$renderer3, {
        ...
        children: ($$renderer4) => {
          $$renderer4.push(`<div><div class="flex justify-between dark:text-gray-300 px-4.5 pt-3 pb-2"><div class="text-sm font-medium self-center flex items-center">`);
          ...
          $$renderer4.push(`<!--]--></div> <button class="self-center rounded-lg p-1 text-gray-500 transition hover:bg-gray-50 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-gray-200"${attr("aria-label", store_get($$store_subs ??= {}, "$i18n", i18n).t("Close citation modal"))}>`);
          ...
          for (let documentIdx = 0, $$length = each_array.length; documentIdx < $$length; documentIdx++) {
            ...
            if (document2.metadata?.parameters) {
              $$renderer4.push("<!--[0-->");
              $$renderer4.push(`<div><div class="text-sm font-normal dark:text-gray-300 mb-1">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Parameters"))}</div> `);
              Textarea($$renderer4, {
                readonly: true,
            ...
            $$renderer4.push(`<!--]--> <div><div class="text-sm font-normal dark:text-gray-300 flex items-center gap-2 w-fit mb-1">`);
                ...
                children: ($$renderer5) => {
                  ...
                  if (showPercentage) {
                    ...
                    if (typeof percentage === "number") {
                      $$renderer5.push("<!--[0-->");
                      $$renderer5.push(`<span${attr_class(`px-1 rounded-sm font-normal ${getRelevanceColor(percentage)}`)}>${escape_html(percentage.toFixed(2))}%</span>`);
                    } else {
                      $$renderer5.push("<!--[-1-->");
                    }
                    $$renderer5.push(`<!--]-->`);
            ...
            if (document2.metadata?.html) {
              $$renderer4.push("<!--[0-->");
              $$renderer4.push(`<iframe class="w-full border-0 h-auto rounded-none"${attr("sandbox", `allow-scripts allow-forms${store_get($$store_subs ??= {}, "$settings", settings)?.iframeSandboxAllowSameOrigin ?? false ? " allow-same-origin" : ""}`)}${attr("srcdoc", injectCsp(document2.document, store_get($$store_subs ??= {}, "$config", config)?.ui?.iframe_csp ?? ""))}${attr("title", store_get($$store_subs ??= {}, "$i18n", i18n).t("Content"))}></iframe>`);
            } else {
              $$renderer4.push("<!--[-1-->");
              ...
              if (store_get($$store_subs ??= {}, "$settings", settings)?.renderMarkdownInPreviews ?? true) {
                $$renderer4.push("<!--[0-->");
                $$renderer4.push(`<div class="text-sm prose dark:prose-invert markdown-prose-sm min-w-full max-w-full">`);
                Markdown($$renderer4, {
  ...
  $$renderer.component(($$renderer2) => {
    ...
    function $$render_inner($$renderer3) {
      ...
      if (citations.length > 0) {
        ...
        $$renderer3.push(`<div class="py-1 -mx-0.5 w-full flex gap-1 items-center flex-wrap"><button class="text-xs font-normal text-gray-600 dark:text-gray-300 px-3.5 h-8 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition flex items-center gap-1 border border-gray-50 dark:border-gray-850/30"${attr("aria-label", citations.length === 1 ? store_get($$store_subs ??= {}, "$i18n", i18n).t("Toggle 1 source") : store_get($$store_subs ??= {}, "$i18n", i18n).t("Toggle {{COUNT}} sources", { COUNT: citations.length }))}${attr("aria-expanded", showCitations)}>`);
        if (urlCitations.length > 0) {
          ...
          if (citations.length > 3) {
            $$renderer3.push("<!--[0-->");
            $$renderer3.push(`<div class="size-4 rounded-full shrink-0 border border-white dark:border-gray-850 bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-[8px] font-normal text-gray-500 dark:text-gray-400 whitespace-nowrap tracking-tighter" aria-hidden="true">+${escape_html(citations.length - Math.min(urlCitations.length, 3))}</div>`);
          } else {
            $$renderer3.push("<!--[-1-->");
          }
          $$renderer3.push(`<!--]--></div>`);
  ...
  $$renderer.component(($$renderer2) => {
    ...
    function $$render_inner($$renderer3) {
      Modal($$renderer3, {
        ...
        children: ($$renderer4) => {
          $$renderer4.push(`<div><div class="flex justify-between dark:text-gray-300 px-4 pt-3 pb-1"><div class="text-sm font-medium self-center flex flex-col gap-0.5 capitalize">`);
          ...
          $$renderer4.push(`<!--]--></div></div></div> <button class="self-center rounded-lg p-1 text-gray-500 transition hover:bg-gray-50 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-gray-200">`);
          XMark($$renderer4, { className: "size-4" });
          ...
          if (codeExecution?.result?.files && codeExecution?.result?.files.length > 0) {
            $$renderer4.push("<!--[0-->");
            $$renderer4.push(`<div class="flex flex-col w-full"><hr class="border-gray-100/30 dark:border-gray-850/30 my-2"/> <div class="text-sm font-normal dark:text-gray-300">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Files"))}</div> <ul class="mt-1 list-disc pl-4 text-xs"><!--[-->`);
    ...
    function $$render_inner($$renderer3) {
      ...
      if (codeExecutions.length > 0) {
        ...
        for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
          ...
          $$renderer3.push(`<!--]--></div> <div${attr_class(`flex-1 mx-2 line-clamp-1 code-execution-name ${execution?.result ? "" : "pulse"}`, "svelte-tclqny")}>${escape_html(execution.name)}</div></button></div>`);
        }
        $$renderer3.push(`<!--]--></div>`);
      } else {
        $$renderer3.push("<!--[-1-->");
...
function FloatingButtons($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let onSetInputText = fallback($$props["onSetInputText"], (text) => {
    });
...
function ContentRenderer($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    const formatMessageContent = (messageContent) => model?.info?.meta?.capabilities?.citations == false ? replaceOutsideCode(messageContent, (segment) => segment.replace(/\s*(\[(?:\d+(?:#[^,\]\s]+)?(?:,\s*\d+(?:#[^,\]\s]+)?)*)\])+/g, "")) : messageContent;
    let autoOpenedArtifactIds = /* @__PURE__ */ new Set();
    const hasClosingCodeFence = (raw = "") => /(?:^|\n)```[ \t]*$/.test(raw.trimEnd());
    const markdownUpdateHandler = (
      /** @type {any} */
      async (token, codeBlockId = "") => {
        const { lang = "", raw = "", text: code = "" } = token;
        const normalizedLang = lang.toLowerCase();
        const isArtifact = ["html", "svg"].includes(normalizedLang) || normalizedLang === "xml" && code.toLowerCase().includes("<svg");
        const artifactId = codeBlockId || `${normalizedLang}:${raw}`;
        if ((store_get($$store_subs ??= {}, "$settings", settings)?.detectArtifacts ?? true) && isArtifact && hasClosingCodeFence(raw) && !autoOpenedArtifactIds.has(artifactId) && !store_get($$store_subs ??= {}, "$mobile", mobile) && store_get($$store_subs ??= {}, "$chatId", chatId)) {
    ...
    if (output?.length) {
      $$renderer2.push("<!--[0-->");
      StructuredOutputRenderer($$renderer2, {
        id,
        output,
        model,
        save,
        ...
        topPadding,
        sourceIds,
        renderMarkdown: store_get($$store_subs ??= {}, "$settings", settings)?.renderMarkdownInAssistantMessages ?? true,
    ...
    } else if (store_get($$store_subs ??= {}, "$settings", settings)?.renderMarkdownInAssistantMessages ?? true) {
      $$renderer2.push("<!--[1-->");
      ...
      if (extracted.plainContent) {
        $$renderer2.push("<!--[0-->");
        $$renderer2.push(`<div class="whitespace-pre-wrap text-[0.9375rem]">${escape_html(extracted.plainContent)}</div>`);
      } else {
        $$renderer2.push("<!--[-1-->");
      }
      $$renderer2.push(`<!--]-->`);
...
function FollowUps($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div class="mt-4"><div class="text-sm font-normal">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Follow up"))}</div> <div class="flex flex-col text-left gap-1 mt-1.5"><!--[-->`);
        ...
        $$slots: {
          ...
          content: ($$renderer4) => {
            ...
            DropdownMenu($$renderer4, {
              className: "max-w-[200px] transition",
              children: ($$renderer5) => {
                $$renderer5.push(`<div class="py-1 px-2 flex dark:text-gray-100"><input type="text" id="floating-message-input" class="bg-transparent outline-hidden w-full flex-1 text-[13px]"${attr("placeholder", store_get($$store_subs ??= {}, "$i18n", i18n).t("Suggest a change"))}${attr("value", inputValue)} autocomplete="off"/> <div class="ml-2 self-center flex items-center"><button${attr("aria-label", store_get($$store_subs ??= {}, "$i18n", i18n).t("Submit suggestion"))}${attr_class(`${"text-white bg-gray-200 dark:text-gray-900 dark:bg-gray-700 disabled"} transition rounded-full p-1 self-center`)}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="size-3.5"><path fill-rule="evenodd" d="M8 14a.75.75 0 0 1-.75-.75V4.56L4.03 7.78a.75.75 0 0 1-1.06-1.06l4.5-4.5a.75.75 0 0 1 1.06 0l4.5 4.5a.75.75 0 0 1-1.06 1.06L8.75 4.56v8.69A.75.75 0 0 1 8 14Z" clip-rule="evenodd"></path></svg></button></div></div> <hr class="border-gray-50/30 dark:border-gray-800/30 my-0.5 mx-1"/> <button class="select-none flex h-[1.6875rem] w-full items-center gap-2 rounded-xl px-2 text-[13px] cursor-pointer hover:bg-gray-50/40 dark:hover:bg-gray-800/40"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" aria-hidden="true" stroke="currentColor" class="size-3.5"><path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99"></path></svg> <div class="flex items-center">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Try Again"))}</div></button> <button class="select-none flex h-[1.6875rem] w-full items-center gap-2 rounded-xl px-2 text-[13px] cursor-pointer hover:bg-gray-50/40 dark:hover:bg-gray-800/40">`);
                LineSpace($$renderer5, { className: "size-3.5", strokeWidth: "2" });
                $$renderer5.push(`<!----> <div class="flex items-center">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Add Details"))}</div></button> <button class="select-none flex h-[1.6875rem] w-full items-center gap-2 rounded-xl px-2 text-[13px] cursor-pointer hover:bg-gray-50/40 dark:hover:bg-gray-800/40">`);
                LineSpaceSmaller($$renderer5, { className: "size-3.5", strokeWidth: "2" });
                $$renderer5.push(`<!----> <div class="flex items-center">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("More Concise"))}</div></button>`);
              },
              $$slots: { default: true }
            });
            $$renderer4.push(`<!----></div>`);
    ...
    if (!status?.hidden) {
      ...
      if (status?.action === "web_search" && (status?.urls || status?.items)) {
        ...
        WebSearchResults($$renderer2, {
          status,
          children: ($$renderer3) => {
            $$renderer3.push(`<div class="flex flex-col justify-center -space-y-0.5"><div${attr_class(`${(done || status?.done) === false ? "shimmer" : ""} text-[0.9375rem] line-clamp-1 text-wrap`)}>`);
        ...
        $$renderer2.push("<!--[1-->");
        $$renderer2.push(`<div class="flex flex-col justify-center -space-y-0.5"><div${attr_class(`${(done || status?.done) === false ? "shimmer" : ""} text-gray-500 dark:text-gray-500 text-[0.9375rem] line-clamp-1 text-wrap`)}>${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t(`Searching Knowledge for "{{searchQuery}}"`, { searchQuery: status.query }))}</div></div>`);
      } else if (status?.action === "web_search_queries_generated" && status?.queries) {
        $$renderer2.push("<!--[2-->");
        $$renderer2.push(`<div class="flex flex-col justify-center -space-y-0.5"><div${attr_class(`${(done || status?.done) === false ? "shimmer" : ""} text-gray-500 dark:text-gray-500 text-[0.9375rem] line-clamp-1 text-wrap`)}>${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t(`Searching`))}</div> <div class="flex gap-1 flex-wrap mt-2"><!--[-->`);
        ...
        $$renderer2.push("<!--[3-->");
        $$renderer2.push(`<div class="flex flex-col justify-center -space-y-0.5"><div${attr_class(`${(done || status?.done) === false ? "shimmer" : ""} text-gray-500 dark:text-gray-500 text-[0.9375rem] line-clamp-1 text-wrap`)}>${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t(`Querying`))}</div> <div class="flex gap-1 flex-wrap mt-2"><!--[-->`);
        ...
        $$renderer2.push("<!--[4-->");
        $$renderer2.push(`<div class="flex flex-col justify-center -space-y-0.5"><div${attr_class(`${(done || status?.done) === false ? "shimmer" : ""} text-gray-500 dark:text-gray-500 text-[0.9375rem] line-clamp-1 text-wrap`)}>`);
        if (status.count === 0) {
          $$renderer2.push("<!--[0-->");
  ...
  $$renderer.component(($$renderer2) => {
    ...
    if (history && history.length > 0) {
      ...
      if (status?.hidden !== true) {
        $$renderer2.push("<!--[0-->");
        $$renderer2.push(`<div class="text-[0.9375rem] flex flex-col w-full"><button class="w-full"${attr("aria-label", store_get($$store_subs ??= {}, "$i18n", i18n).t("Toggle status history"))}${attr("aria-expanded", showHistory)}><div class="flex items-start gap-2">`);
...
function ResponseMessage($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    let statusEntries, hasVisibleStatus, visibleResponseContent, hasResponseContent;
    const i18n = getContext("i18n");
    const dispatch = createEventDispatcher();
    let chatId2 = fallback($$props["chatId"], "");
    let history = $$props["history"];
    ...
    let forkHandler = fallback($$props["forkHandler"], null);
    let addMessages = $$props["addMessages"];
    let isLastMessage = fallback($$props["isLastMessage"], true);
    let readOnly = fallback($$props["readOnly"], false);
    let allowDelete = fallback($$props["allowDelete"], true);
    let compactPreview = fallback($$props["compactPreview"], false);
    let editCodeBlock = fallback($$props["editCodeBlock"], true);
    let topPadding = fallback($$props["topPadding"], false);
    ...
    if (history.messages) {
      ...
      if (source) {
        if (message.content !== source.content || message.done !== source.done || message.output?.length !== source.output?.length) {
    ...
    function $$render_inner($$renderer3) {
      ...
      $$renderer3.push(`<!----> <!---->`);
      {
        $$renderer3.push(`<div${attr_class(` flex w-full message-${stringify(message.id)}`, "svelte-1qscqw6")}${attr("id", `message-${stringify(message.id)}`)}${attr("dir", store_get($$store_subs ??= {}, "$settings", settings).chatDirection)} style="scroll-margin-top: 3rem;"><div${attr_class(`shrink-0 ltr:mr-2 rtl:ml-2 hidden @lg:flex mt-0.5 `, "svelte-1qscqw6")}>`);
        ...
        $$renderer3.push(`<!--]--> <div><div${attr_class(`chat-${stringify(message.role)} w-full min-w-full`, "svelte-1qscqw6")}><div>`);
        ...
        if (message?.files && message.files?.filter((f) => ["image", "file"].includes(f.type)).length > 0) {
          ...
          const each_array = ensure_array_like(message.files.filter((f) => ["image", "file"].includes(f.type)));
          for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
            let file = each_array[$$index];
            $$renderer3.push(`<div>`);
        ...
        $$renderer3.push(`<!--]--> <div${attr_class(`w-full flex flex-col relative ${""}`)} id="response-content-container">`);
          ...
          ContentRenderer($$renderer3, {
            ...
            onSave: ({ raw, oldContent, newContent }) => {
              ...
              if (sourceMessage.output?.length) {
                const updatedOutput = replaceOutputMessageText(sourceMessage.output, oldContent, newContent);
        ...
        if (compactPreview && message.timestamp) {
          $$renderer3.push("<!--[0-->");
          $$renderer3.push(`<div class="mt-0.5 flex justify-start whitespace-nowrap text-gray-600 dark:text-gray-500">`);
          Tooltip($$renderer3, {
            className: "flex self-center",
            content: formatMessageTimestampFull(message.timestamp * 1e3),
            placement: "bottom",
            children: ($$renderer4) => {
              $$renderer4.push(`<time${attr("datetime", new Date(message.timestamp * 1e3).toISOString())} class="ml-1 shrink-0 whitespace-nowrap text-[0.6875rem] tabular-nums text-gray-400 dark:text-gray-600 select-none">${escape_html(formatMessageTimestamp(message.timestamp * 1e3))}</time>`);
            },
            $$slots: { default: true }
          });
          $$renderer3.push(`<!----></div>`);
          ...
          $$renderer3.push("<!--[1-->");
          $$renderer3.push(`<div class="flex items-center justify-start overflow-x-auto whitespace-nowrap buttons text-gray-600 dark:text-gray-500 mt-0.5 [&amp;>*]:shrink-0 svelte-1qscqw6">`);
          if (message.done || siblings.length > 1) {
            ...
            if (siblings.length > 1) {
              ...
              {
                $$renderer3.push("<!--[-1-->");
                $$renderer3.push(`<div class="text-sm tracking-widest font-normal self-center dark:text-gray-100 min-w-fit">${escape_html(siblings.indexOf(message.id) + 1)}/${escape_html(siblings.length)}</div>`);
              }
            ...
            if (message.done) {
              ...
              if (onInsertToNote && visibleResponseContent) {
                $$renderer3.push("<!--[0-->");
                Tooltip($$renderer3, {
                  content: store_get($$store_subs ??= {}, "$i18n", i18n).t("Insert into note"),
                  placement: "bottom",
                  children: ($$renderer4) => {
                    $$renderer4.push(`<button${attr("aria-label", store_get($$store_subs ??= {}, "$i18n", i18n).t("Insert into note"))}${attr_class(`${isLastMessage || (store_get($$store_subs ??= {}, "$settings", settings)?.highContrastMode ?? false) ? "visible" : "invisible group-hover:visible"} rounded-lg px-2 py-1.5 text-xs text-gray-500 transition hover:bg-black/5 hover:text-black dark:hover:bg-white/5 dark:hover:text-white`)}>${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Insert"))}</button>`);
                  },
                  $$slots: { default: true }
                });
              } else {
              ...
              if (!readOnly && (store_get($$store_subs ??= {}, "$user", user)?.role === "admin" || (store_get($$store_subs ??= {}, "$user", user)?.permissions?.chat?.tts ?? true))) {
                ...
                Tooltip($$renderer3, {
                  ...
                  placement: "bottom",
                  children: ($$renderer4) => {
                    $$renderer4.push(`<button${attr("aria-label", store_get($$store_subs ??= {}, "$i18n", i18n).t("Read Aloud"))}${attr("id", `speak-button-${stringify(message.id)}`)}${attr_class(`${isLastMessage || (store_get($$store_subs ??= {}, "$settings", settings)?.highContrastMode ?? false) ? "visible" : "invisible group-hover:visible"} p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg dark:hover:text-white hover:text-black transition`)}>`);
                    {
                      $$renderer4.push("<!--[-1-->");
              ...
              if (!readOnly) {
                    ...
                    RegenerateMenu($$renderer3, {
                      ...
                      children: ($$renderer4) => {
                        Tooltip($$renderer4, {
                          ...
                          placement: "bottom",
                          children: ($$renderer5) => {
                            $$renderer5.push(`<button type="button"${attr("aria-label", store_get($$store_subs ??= {}, "$i18n", i18n).t("Regenerate"))}${attr_class(`${isLastMessage ? "visible" : "invisible group-hover:visible"} p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg dark:hover:text-white hover:text-black transition`)}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.3" aria-hidden="true" stroke="currentColor" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99"></path></svg></button>`);
                          },
                          $$slots: { default: true }
                        });
                      },
                ...
                for (let $$index_2 = 0, $$length = each_array_2.length; $$index_2 < $$length; $$index_2++) {
                  ...
                  Tooltip($$renderer3, {
                    ...
                    placement: "bottom",
                    children: ($$renderer4) => {
                      $$renderer4.push(`<button type="button"${attr("aria-label", action.name)}${attr_class(`${isLastMessage || (store_get($$store_subs ??= {}, "$settings", settings)?.highContrastMode ?? false) ? "visible" : "invisible group-hover:visible"} p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg dark:hover:text-white hover:text-black transition`)}>`);
                      if (action?.icon) {
                        $$renderer4.push("<!--[0-->");
                        $$renderer4.push(`<div class="size-4"><img${attr("src", action.icon)}${attr_class(`w-4 h-4 ${action.icon.includes("data:image/svg") ? "dark:invert-[80%]" : ""}`)} style="fill: currentColor;"${attr("alt", action.name)} draggable="false"/></div>`);
                      } else {
                        $$renderer4.push("<!--[-1-->");
                ...
                if (message.done && !readOnly && forkHandler) {
                  ...
                  Tooltip($$renderer3, {
                    ...
                    placement: "bottom",
                    children: ($$renderer4) => {
                      $$renderer4.push(`<button aria-label="Fork chat"${attr_class(`${isLastMessage || (store_get($$store_subs ??= {}, "$settings", settings)?.highContrastMode ?? false) ? "visible" : "invisible group-hover:visible"} p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg dark:hover:text-white hover:text-black transition`)}><svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 12H9"></path><path d="M9 12C12.5 12 12.5 7 16 7H20"></path><path d="M17 4L20 7L17 10"></path><path d="M9 12C12.5 12 12.5 17 16 17H20"></path><path d="M17 14L20 17L17 20"></path></svg></button>`);
                    },
                    $$slots: { default: true }
                  });
                } else {
                ...
                if (message.timestamp) {
                  ...
                  Tooltip($$renderer3, {
                    ...
                    placement: "bottom",
                    children: ($$renderer4) => {
                      $$renderer4.push(`<time${attr("datetime", new Date(message.timestamp * 1e3).toISOString())} class="invisible group-hover:visible ml-1 shrink-0 whitespace-nowrap text-[0.6875rem] tabular-nums text-gray-400 dark:text-gray-600 select-none">${escape_html(formatMessageTimestamp(message.timestamp * 1e3))}</time>`);
                    },
                    $$slots: { default: true }
                  });
                } else {
          ...
          if ((isLastMessage || (store_get($$store_subs ??= {}, "$settings", settings)?.keepFollowUpPrompts ?? false)) && message.done && !readOnly && (message?.followUps ?? []).length > 0) {
            $$renderer3.push("<!--[0-->");
            $$renderer3.push(`<div class="my-2.5">`);
            FollowUps($$renderer3, {
...
function SubagentResultRow($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    let delegationIds, delegationLabel, summary;
    const i18n = getContext("i18n");
    let content = $$props["content"];
    let result = $$props["result"];
    let expanded = false;
    delegationIds = Array.isArray(result.delegation_ids) ? result.delegation_ids : [];
    delegationLabel = result.delegation_id ?? (delegationIds.length > 1 ? `${delegationIds.length} tasks` : "");
    summary = (() => {
    ...
    $$renderer2.push(`<div class="w-full min-w-0 pb-1"><button type="button" class="w-full min-w-0 flex items-center gap-2 text-left text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 transition-colors"${attr("aria-expanded", expanded)}><span class="text-[0.75rem] font-normal shrink-0">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Background sub-agent finished"))}</span> `);
    if (summary) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<span class="text-[0.75rem] truncate min-w-0 flex-1">${escape_html(summary)}</span>`);
    } else {
      $$renderer2.push("<!--[-1-->");
    }
    $$renderer2.push(`<!--]--> `);
    if (delegationLabel) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<span class="hidden sm:inline text-[0.6875rem] font-mono text-gray-400 dark:text-gray-600 shrink-0">${escape_html(delegationLabel)}</span>`);
    } else {
      $$renderer2.push("<!--[-1-->");
    }
    $$renderer2.push(`<!--]--> `);
    ChevronDown($$renderer2, {
      className: `size-3 text-gray-400 dark:text-gray-600 shrink-0 transition-transform duration-150 ${""}`
    });
    $$renderer2.push(`<!----></button> `);
    {
      $$renderer2.push("<!--[-1-->");
    ...
    $$renderer2.push(`<!--]--></div>`);
    if ($$store_subs) unsubscribe_stores($$store_subs);
    bind_props($$props, { content, result });
  });
}
function UserMessage($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    function $$render_inner($$renderer3) {
      ...
      if (!(store_get($$store_subs ??= {}, "$settings", settings)?.chatBubble ?? true) && !(message?.meta?.internal === true && message?.meta?.type === "subagent") && !(message?.meta?.internal === true && message?.meta?.type === "timer")) {
        $$renderer3.push("<!--[0-->");
        $$renderer3.push(`<div${attr_class(`shrink-0 ltr:mr-2 rtl:ml-2 hidden @lg:flex mt-0.5`)}>`);
        ProfileImage($$renderer3, {
      ...
      $$renderer3.push(`<!--]--> <div${attr_class(`flex-auto w-0 max-w-full ${message?.meta?.internal === true && message?.meta?.type === "subagent" || message?.meta?.internal === true && message?.meta?.type === "timer" ? "" : "pl-1"}`)}>`);
      if (!(store_get($$store_subs ??= {}, "$settings", settings)?.chatBubble ?? true) && !(message?.meta?.internal === true && message?.meta?.type === "subagent") && !(message?.meta?.internal === true && message?.meta?.type === "timer")) {
        $$renderer3.push("<!--[0-->");
        $$renderer3.push(`<div>`);
        Name($$renderer3, {
          children: ($$renderer4) => {
            if (message.user) {
              $$renderer4.push("<!--[0-->");
              $$renderer4.push(`${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("You"))} <span class="text-gray-500 text-[0.9375rem] font-normal">${escape_html(message?.user ?? "")}</span>`);
      ...
      $$renderer3.push(`<!--]--> <div${attr_class(`chat-${stringify(message.role)} w-full min-w-full`)}>`);
      {
        $$renderer3.push("<!--[0-->");
        if (message.files) {
          $$renderer3.push("<!--[0-->");
      ...
      $$renderer3.push(`<!--]--> `);
      if (message?.meta?.internal === true && message?.meta?.type === "timer") {
        $$renderer3.push("<!--[1-->");
        $$renderer3.push(`<div class="w-full min-w-0"><button type="button" class="flex w-full min-w-0 items-center gap-2 text-left text-gray-500 transition-colors hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"${attr("aria-expanded", timerExpanded)}><span class="shrink-0 text-[0.75rem] font-medium">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Timer"))}</span> <span class="min-w-0 flex-1 truncate text-[0.75rem]">${escape_html(message.content)}</span> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"${attr_class(`size-3 shrink-0 text-gray-400 transition-transform duration-150 dark:text-gray-600 ${""}`)}><path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5"></path></svg></button> `);
        {
          $$renderer3.push("<!--[-1-->");
        }
        $$renderer3.push(`<!--]--></div>`);
        ...
        $$renderer3.push("<!--[2-->");
        SubagentResultRow($$renderer3, { content: message.content, result: message.meta });
      } else if (message.content !== "") {
        $$renderer3.push("<!--[3-->");
        $$renderer3.push(`<div class="w-full"><div${attr_class(`flex ${store_get($$store_subs ??= {}, "$settings", settings)?.chatBubble ?? true ? "justify-end pb-1" : "w-full"}`)}><div${attr_class(`rounded-3xl ${store_get($$store_subs ??= {}, "$settings", settings)?.chatBubble ?? true ? `max-w-[90%] px-4 py-1.5  bg-gray-50 dark:bg-gray-850 ${message.files ? "rounded-tr-lg" : ""}` : " w-full"}`)}>`);
        if (message.content) {
          $$renderer3.push("<!--[0-->");
          if (store_get($$store_subs ??= {}, "$settings", settings)?.renderMarkdownInUserMessages ?? true) {
            $$renderer3.push("<!--[0-->");
            $$renderer3.push(`<div class="markdown-prose">`);
            Markdown($$renderer3, {
              id: `${chatId2}-${message.id}`,
              content: message.content,
              editCodeBlock,
            ...
            $$renderer3.push("<!--[-1-->");
            $$renderer3.push(`<div class="whitespace-pre-wrap text-[0.9375rem]"${attr("dir", store_get($$store_subs ??= {}, "$settings", settings)?.chatDirection ?? "auto")}>${escape_html(message.content)}</div>`);
          }
          $$renderer3.push(`<!--]-->`);
        } else {
          $$renderer3.push("<!--[-1-->");
      ...
      $$renderer3.push(`<!--]--> `);
      if (!(message?.meta?.internal === true && message?.meta?.type === "subagent") && !(message?.meta?.internal === true && message?.meta?.type === "timer")) {
        $$renderer3.push("<!--[0-->");
        $$renderer3.push(`<div${attr_class(` flex ${store_get($$store_subs ??= {}, "$settings", settings)?.chatBubble ?? true ? "justify-end" : "items-center"} text-gray-600 dark:text-gray-500`)}>`);
        if (message.timestamp) {
          $$renderer3.push("<!--[0-->");
          Tooltip($$renderer3, {
            className: `flex self-center ${store_get($$store_subs ??= {}, "$settings", settings)?.chatBubble ?? true ? "mr-1" : "order-last"}`,
            content: formatMessageTimestampFull(message.timestamp * 1e3),
            placement: "bottom",
            children: ($$renderer4) => {
              $$renderer4.push(`<time${attr("datetime", new Date(message.timestamp * 1e3).toISOString())}${attr_class(`${compactPreview ? "" : "invisible group-hover:visible"} ${store_get($$store_subs ??= {}, "$settings", settings)?.chatBubble ?? true ? "mr-1" : "ml-1 shrink-0 whitespace-nowrap"} text-[0.6875rem] tabular-nums text-gray-400 dark:text-gray-600 select-none`)}>${escape_html(formatMessageTimestamp(message.timestamp * 1e3))}</time>`);
            },
            $$slots: { default: true }
          });
        } else {
        ...
        $$renderer3.push(`<!--]--> `);
        if (!compactPreview && !(store_get($$store_subs ??= {}, "$settings", settings)?.chatBubble ?? true)) {
          $$renderer3.push("<!--[0-->");
          if (siblings.length > 1) {
            $$renderer3.push("<!--[0-->");
            $$renderer3.push(`<div class="flex self-center" dir="ltr"><button${attr("aria-label", store_get($$store_subs ??= {}, "$i18n", i18n).t("Previous message"))} class="self-center p-1 hover:bg-black/5 dark:hover:bg-white/5 dark:hover:text-white hover:text-black rounded-md transition"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" class="size-3.5"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5"></path></svg></button> `);
            {
              $$renderer3.push("<!--[-1-->");
              $$renderer3.push(`<div class="text-sm tracking-widest font-normal self-center dark:text-gray-100 min-w-fit">${escape_html(siblings.indexOf(message.id) + 1)}/${escape_html(siblings.length)}</div>`);
            }
            $$renderer3.push(`<!--]--> <button${attr("aria-label", store_get($$store_subs ??= {}, "$i18n", i18n).t("Next message"))} class="self-center p-1 hover:bg-black/5 dark:hover:bg-white/5 dark:hover:text-white hover:text-black rounded-md transition"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" class="size-3.5"><path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5"></path></svg></button></div>`);
          } else {
            $$renderer3.push("<!--[-1-->");
          }
          $$renderer3.push(`<!--]-->`);
        ...
        if (!compactPreview && !readOnly) {
          ...
          Tooltip($$renderer3, {
            ...
            placement: "bottom",
            children: ($$renderer4) => {
              $$renderer4.push(`<button${attr_class(`${store_get($$store_subs ??= {}, "$settings", settings)?.highContrastMode ?? false ? "" : "invisible group-hover:visible"} p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg dark:hover:text-white hover:text-black transition edit-user-message-button`)}${attr("aria-label", store_get($$store_subs ??= {}, "$i18n", i18n).t("Edit"))}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.3" stroke="currentColor" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125"></path></svg></button>`);
            },
            $$slots: { default: true }
          });
        } else {
        ...
        if (!compactPreview && message?.content) {
          ...
          Tooltip($$renderer3, {
            ...
            placement: "bottom",
            children: ($$renderer4) => {
              $$renderer4.push(`<button${attr_class(`${store_get($$store_subs ??= {}, "$settings", settings)?.highContrastMode ?? false ? "" : "invisible group-hover:visible"} p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg dark:hover:text-white hover:text-black transition`)}${attr("aria-label", store_get($$store_subs ??= {}, "$i18n", i18n).t("Copy"))}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.3" stroke="currentColor" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="M15.666 3.888A2.25 2.25 0 0013.5 2.25h-3c-1.03 0-1.9.693-2.166 1.638m7.332 0c.055.194.084.4.084.612v0a.75.75 0 01-.75.75H9a.75.75 0 01-.75-.75v0c0-.212.03-.418.084-.612m7.332 0c.646.049 1.288.11 1.927.184 1.1.128 1.907 1.077 1.907 2.185V19.5a2.25 2.25 0 01-2.25 2.25H6.75A2.25 2.25 0 014.5 19.5V6.257c0-1.108.806-2.057 1.907-2.185a48.208 48.208 0 011.927-.184"></path></svg></button>`);
            },
            $$slots: { default: true }
          });
        } else {
        ...
        if (store_get($$store_subs ??= {}, "$_user", user)?.role === "admin" || (store_get($$store_subs ??= {}, "$_user", user)?.permissions?.chat?.delete_message ?? false)) {
          ...
          if (!compactPreview && !readOnly && allowDelete && (!isFirstMessage || siblings.length > 1)) {
            ...
            Tooltip($$renderer3, {
              ...
              placement: "bottom",
              children: ($$renderer4) => {
                $$renderer4.push(`<button${attr("aria-label", store_get($$store_subs ??= {}, "$i18n", i18n).t("Delete"))}${attr_class(`${store_get($$store_subs ??= {}, "$settings", settings)?.highContrastMode ?? false ? "" : "invisible group-hover:visible"} p-1 rounded-sm dark:hover:text-white hover:text-black transition`)}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"></path></svg></button>`);
              },
              $$slots: { default: true }
            });
          } else {
...
function Message($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let $$inner_renderer;
    function $$render_inner($$renderer3) {
      $$renderer3.push(`<div role="listitem"${attr_class(`flex flex-col justify-between px-3.5 mb-3 w-full ${store_get($$store_subs ??= {}, "$settings", settings)?.widescreenMode ?? null ? "max-w-full" : "max-w-[58rem]"} mx-auto rounded-lg group ${isSafari ? "" : "message-listitem"}`, "svelte-1tkqtxv")}>`);
...
function Suggestions($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    getFilteredPrompts(inputValue);
    $$renderer2.push(`<div class="mb-1 flex gap-1 text-xs font-normal items-center text-gray-600 dark:text-gray-400 svelte-6tnq1o">`);
    if (filteredPrompts.length > 0) {
      ...
      $$renderer2.push("<!--[-1-->");
      $$renderer2.push(`<div${attr_class(`flex w-full ${store_get($$store_subs ??= {}, "$settings", settings)?.landingPageMode === "chat" ? " -mt-1" : "text-center items-center justify-center"} self-start text-gray-600 dark:text-gray-400`, "svelte-6tnq1o")}>${escape_html(store_get($$store_subs ??= {}, "$WEBUI_NAME", WEBUI_NAME))} ‧ v${escape_html(WEBUI_VERSION)}</div>`);
    }
    $$renderer2.push(`<!--]--></div> <div class="h-36 w-full svelte-6tnq1o">`);
    if (filteredPrompts.length > 0) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<div role="list"${attr_class(`max-h-36 overflow-auto scrollbar-none items-start ${stringify(className)}`, "svelte-6tnq1o")}><!--[-->`);
      const each_array = ensure_array_like(filteredPrompts);
      for (let idx = 0, $$length = each_array.length; idx < $$length; idx++) {
        let prompt = each_array[idx];
        $$renderer2.push(`<button role="listitem" class="waterfall flex flex-col flex-1 shrink-0 w-full justify-between px-2.5 py-1.5 rounded-lg bg-transparent transition-colors hover:text-gray-950 dark:hover:text-white group svelte-6tnq1o"${attr_style(`animation-delay: ${stringify(idx * 45)}ms`)}><div class="flex flex-col text-left leading-snug svelte-6tnq1o">`);
        if (prompt.title && prompt.title[0] !== "") {
          $$renderer2.push("<!--[0-->");
          $$renderer2.push(`<div class="text-sm font-normal group-hover:text-gray-950 dark:text-gray-300 dark:group-hover:text-white transition line-clamp-1 svelte-6tnq1o">${escape_html(prompt.title[0])}</div> <div class="text-xs text-gray-600 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-gray-100 font-normal line-clamp-1 svelte-6tnq1o">${escape_html(prompt.title[1])}</div>`);
        } else {
          $$renderer2.push("<!--[-1-->");
          $$renderer2.push(`<div class="text-sm font-normal group-hover:text-gray-950 dark:text-gray-300 dark:group-hover:text-white transition line-clamp-1 svelte-6tnq1o">${escape_html(prompt.content)}</div> <div class="text-xs text-gray-600 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-gray-100 font-normal line-clamp-1 svelte-6tnq1o">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Prompt"))}</div>`);
        }
        $$renderer2.push(`<!--]--></div></button>`);
      }
      $$renderer2.push(`<!--]--></div>`);
...
function ChatPlaceholder($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<!---->`);
    {
      $$renderer2.push(`<div class="m-auto w-full max-w-[58rem] px-8 lg:px-20"><div class="flex justify-start"><div class="flex -space-x-4 mb-0.5"><!--[-->`);
      ...
      for (let modelIdx = 0, $$length = each_array.length; modelIdx < $$length; modelIdx++) {
        ...
        Tooltip($$renderer2, {
          ...
          placement: "right",
          children: ($$renderer3) => {
            $$renderer3.push(`<img${attr("src", `${WEBUI_API_BASE_URL}/models/model/profile/image?id=${model?.id}&lang=${store_get($$store_subs ??= {}, "$i18n", i18n).language}`)} class="size-[2.7rem] rounded-full" alt="logo" draggable="false"/>`);
          },
          $$slots: { default: true }
        });
        $$renderer2.push(`<!----></button>`);
      ...
      if (store_get($$store_subs ??= {}, "$temporaryChatEnabled", temporaryChatEnabled)) {
        ...
        Tooltip($$renderer2, {
          ...
          placement: "top",
          children: ($$renderer3) => {
            $$renderer3.push(`<div class="flex items-center gap-1.5 text-gray-500 text-xs mt-1 w-fit">`);
            EyeSlash($$renderer3, { strokeWidth: "2", className: "size-3.5" });
      ...
      $$renderer2.push(`<!--]--> <div class="mt-2 mb-4 text-3xl text-gray-800 dark:text-gray-100 text-left flex items-center gap-4"><div><div class="capitalize line-clamp-1">`);
      ...
      if (models$1[selectedModelIdx]?.info?.meta?.description ?? null) {
        $$renderer2.push("<!--[0-->");
        $$renderer2.push(`<div class="mt-0.5 text-base font-normal text-gray-500 dark:text-gray-400 line-clamp-3 markdown">${html(DOMPurify.sanitize(marked.parse(sanitizeResponseContent(models$1[selectedModelIdx]?.info?.meta?.description).replaceAll("\n", "<br>"))))}</div> `);
      ...
      $$renderer2.push(`<!--]--></div></div></div> <div class="w-full">`);
...
function Messages($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    const i18n = getContext("i18n");
    let className = fallback($$props["className"], "h-full flex pt-18");
    let chatId2 = fallback($$props["chatId"], "");
    ...
    let messagesContainerId = fallback($$props["messagesContainerId"], "messages-container");
    let onSelect = fallback($$props["onSelect"], (e) => {
    });
    let onInsertToNote = fallback($$props["onInsertToNote"], null);
    let messagesCount = fallback($$props["messagesCount"], 8);
    ...
    const updateChat = async () => {
      if (!store_get($$store_subs ??= {}, "$temporaryChatEnabled", temporaryChatEnabled)) {
        history = history;
        await tick();
        const res = await updateChatById(localStorage.token, chatId2, { history, messages });
    ...
    const gotoMessage = async (message, idx) => {
      ...
      if (store_get($$store_subs ??= {}, "$settings", settings)?.scrollOnBranchChange ?? true) {
        const element = getMessagesContainer();
        autoScroll = element ? element.scrollHeight - element.scrollTop <= element.clientHeight + 50 : false;
        setTimeout(
          () => {
            scrollToBottom();
          },
    ...
    const deleteMessage = async (messageId) => {
      ...
      let nextChildrenIds = nextMessageId === null ? Object.keys(history.messages).filter((id) => history.messages[id].parentId === null) : history.messages[nextMessageId]?.childrenIds ?? [];
      ...
      history = history;
      if (!store_get($$store_subs ??= {}, "$temporaryChatEnabled", temporaryChatEnabled)) {
        const res = await deleteChatMessageById(localStorage.token, chatId2, messageId);
        if (res?.chat?.history) {
