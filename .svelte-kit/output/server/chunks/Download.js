...
function Collapsible($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let chevronClassName = fallback($$props["chevronClassName"], "size-3");
    let chevronStrokeWidth = fallback($$props["chevronStrokeWidth"], "2.75");
    ...
    if (title !== null) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<button type="button"${attr_class(`${stringify(buttonClassName)} block text-start disabled:cursor-default`)}${attr("aria-expanded", open)}${attr("disabled", disabled, true)}><div${attr_class(` w-full flex items-center justify-between gap-2 ${attributes?.done && attributes?.done !== "true" && !messageDone ? "shimmer" : ""} `)}>`);
      ...
      if (!disabled) {
        ...
        if (open) {
          $$renderer2.push("<!--[0-->");
          ChevronUp($$renderer2, { strokeWidth: chevronStrokeWidth, className: chevronClassName });
        } else {
          $$renderer2.push("<!--[-1-->");
          ChevronDown($$renderer2, { strokeWidth: chevronStrokeWidth, className: chevronClassName });
        }
        $$renderer2.push(`<!--]--></div>`);
      } else {
        $$renderer2.push("<!--[-1-->");
