...
import { c as getUserInfoById } from "./UserStatusLinkPreview.js";
...
function AddAccessModal($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let allowGroups = fallback($$props["allowGroups"], true);
    ...
    function $$render_inner($$renderer3) {
      Modal($$renderer3, {
        ...
        children: ($$renderer4) => {
          $$renderer4.push(`<div><div class="flex justify-between dark:text-gray-100 px-4 pt-3 mb-1"><div class="self-center text-sm font-medium"><div class="flex items-center gap-0.5 shrink-0">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Add Access"))}</div></div> <button class="self-center rounded-lg p-1 text-gray-500 transition hover:bg-gray-50 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-gray-200">`);
          XMark($$renderer4, { className: "size-4" });
          $$renderer4.push(`<!----></button></div> <div class="flex flex-col md:flex-row w-full px-3 pb-3 md:space-x-4 dark:text-gray-200"><div class="flex flex-col w-full sm:flex-row sm:justify-center sm:space-x-6"><form class="flex flex-col w-full"><div class="flex flex-col w-full h-full pb-2">`);
          ...
          $$renderer4.push(`<!----></div> <div class="flex justify-end pt-2 text-sm font-normal gap-1.5"><button class="px-3 py-1.5 text-sm font-normal bg-black hover:bg-gray-950 text-white dark:bg-white dark:text-black dark:hover:bg-gray-100 transition rounded-full flex flex-row space-x-1 items-center" type="submit">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Add"))}</button></div></form></div></div></div>`);
        },
        $$slots: { default: true }
      });
    }
    ...
    bind_props($$props, { show, shareUsers, allowGroups, accessGrants, onAdd });
  });
}
function AccessControl($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let shareUsers = fallback($$props["shareUsers"], true);
    let allowGroups = fallback($$props["allowGroups"], true);
    let defaultPermission = fallback($$props["defaultPermission"], "read");
    let groups = [];
    ...
    const hasAnyoneReadGrant = (grants) => grants.some((grant) => grant.principal_type === "anyone" && grant.principal_id === "*" && grant.permission === "read");
    const hasPublicWriteGrant = (grants) => grants.some((grant) => grant.principal_type === "user" && grant.principal_id === "*" && grant.permission === "write");
    const currentGrants = (grants = accessGrants) => Array.isArray(grants) ? grants : [];
    const getPrincipalIdsByPermission = (principalType, permission, grants = accessGrants) => Array.from(new Set(currentGrants(grants).filter((grant) => grant.principal_type === principalType && grant.permission === permission).map((grant) => grant.principal_id)));
    const commitAccessGrants = (nextGrants) => {
      accessGrants = dedupeAccessGrants(nextGrants);
      onChange(accessGrants);
    };
    const getVisibility = (grants) => {
    ...
    readUserIds = getPrincipalIdsByPermission("user", "read", accessGrants).filter((id) => id !== "*");
    writeUserIds = getPrincipalIdsByPermission("user", "write", accessGrants).filter((id) => id !== "*");
    ...
    function $$render_inner($$renderer3) {
      ...
      $$renderer3.push(`<!----> <div class="rounded-lg flex flex-col gap-1"><div class="py-1.5"><div class="flex gap-2 items-center"><div><div class="p-2 bg-black/5 dark:bg-white/5 rounded-full">`);
      if (getVisibility(accessGrants ?? []) === "private") {
        $$renderer3.push("<!--[0-->");
        $$renderer3.push(`<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5"><path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z"></path></svg>`);
      } else {
        $$renderer3.push("<!--[-1-->");
        $$renderer3.push(`<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5"><path stroke-linecap="round" stroke-linejoin="round" d="M6.115 5.19l.319 1.913A6 6 0 008.11 10.36L9.75 12l-.387.775c-.217.433-.132.956.21 1.298l1.348 1.348c.21.21.329.497.329.795v1.089c0 .426.24.815.622 1.006l.153.076c.433.217.956.132 1.298-.21l.723-.723a8.7 8.7 0 002.288-4.042 1.087 1.087 0 00-.358-1.099l-1.33-1.108c-.251-.21-.582-.299-.905-.245l-1.17.195a1.125 1.125 0 01-.98-.314l-.295-.295a1.125 1.125 0 010-1.591l.13-.132a1.125 1.125 0 011.3-.21l.603.302a.809.809 0 001.086-1.086L14.25 7.5l1.256-.837a4.5 4.5 0 001.528-1.732l.146-.292M6.115 5.19A9 9 0 1017.18 4.64M6.115 5.19A8.965 8.965 0 0112 3c1.929 0 3.716.607 5.18 1.64"></path></svg>`);
      }
      $$renderer3.push(`<!--]--></div></div> <div>`);
      Tooltip($$renderer3, {
        content: !(share && sharePublic) && getVisibility(accessGrants ?? []) === "private" ? store_get($$store_subs ??= {}, "$i18n", i18n).t("You do not have permission to make this public") : "",
        children: ($$renderer4) => {
          $$renderer4.select(
            {
              id: "models",
            ...
            ($$renderer5) => {
              ...
              if (share && shareOpen || hasAnyoneReadGrant(accessGrants ?? [])) {
                $$renderer5.push("<!--[0-->");
                $$renderer5.option({ class: "text-gray-700", value: "open" }, ($$renderer6) => {
                  $$renderer6.push(`${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Open"))}`);
                });
              } else {
                $$renderer5.push("<!--[-1-->");
              }
      ...
      $$renderer3.push(`<!----> <div class="text-xs text-gray-400 font-normal">`);
      if (getVisibility(accessGrants ?? []) === "private") {
        $$renderer3.push("<!--[0-->");
        ...
        $$renderer3.push("<!--[-1-->");
        $$renderer3.push(`${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Anyone with the link can view"))}`);
      }
      $$renderer3.push(`<!--]--></div></div></div> `);
      if (hasPublicReadGrant(accessGrants ?? []) && !hasAnyoneReadGrant(accessGrants ?? []) && accessRoles.includes("write")) {
        $$renderer3.push("<!--[0-->");
        $$renderer3.push(`<div class="flex w-full justify-between mt-1.5 ml-0.5"><div class="self-center text-xs">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Allow public write access"))}</div> `);
      ...
      if (share) {
        $$renderer3.push("<!--[0-->");
        $$renderer3.push(`<div class="flex items-center justify-between text-xs font-normal text-gray-500 my-0.5"><div>${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Access List"))}</div> <div class="flex gap-1"><button class="px-2 py-1 bg-transparent hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition text-xs font-normal flex items-center gap-1" type="button">`);
        Plus($$renderer3, { className: "size-3" });
        $$renderer3.push(`<!----> ${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Add Access"))}</button></div></div> <div class="flex flex-col gap-1"><!--[-->`);
        const each_array = ensure_array_like(accessGroups);
        for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
          let group = each_array[$$index];
          $$renderer3.push(`<div class="flex items-center gap-2 justify-between text-sm w-full transition pb-1"><div class="flex items-center gap-2 min-w-0 flex-1"><div class="size-5 rounded-full bg-gray-100 dark:bg-gray-850 flex items-center justify-center text-xs">${escape_html(group.name.charAt(0).toUpperCase())}</div> <div class="truncate text-sm flex items-center gap-2">${escape_html(group.name)} <span class="text-xs text-gray-400 font-normal">${escape_html(group?.member_count)} ${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("members"))}</span></div></div> <div class="flex justify-end items-center gap-1.5 shrink-0">`);
          if (accessRoles.includes("write")) {
            $$renderer3.push("<!--[0-->");
            $$renderer3.select(
              {
                "aria-label": store_get($$store_subs ??= {}, "$i18n", i18n).t("Access level"),
                class: "bg-transparent text-sm outline-none",
                value: writeGroupIds.includes(group.id) ? "write" : "read"
              },
              ($$renderer4) => {
                $$renderer4.option({ value: "read" }, ($$renderer5) => {
                  $$renderer5.push(`${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Read"))}`);
                });
                $$renderer4.option({ value: "write" }, ($$renderer5) => {
                  $$renderer5.push(`${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Write"))}`);
                });
              }
            );
          } else {
          ...
          $$renderer3.push(`<!--]--> <button class="rounded-full p-1 hover:bg-gray-100 dark:hover:bg-gray-850 transition" type="button">`);
        ...
        if (shareUsers) {
          ...
          for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
            ...
            $$renderer3.push(`<div class="flex items-center gap-2 justify-between text-sm w-full transition border-b border-gray-50 dark:border-gray-850 pb-1.5 last:border-0"><div class="flex items-center gap-2 min-w-0 flex-1"><img class="rounded-full size-5 object-cover"${attr("src", `${WEBUI_API_BASE_URL}/users/${user.id}/profile/image`)}${attr("alt", user.name ?? user.id)}/> <div class="min-w-0 flex-1">`);
            Tooltip($$renderer3, {
              content: user.email,
              placement: "top-start",
              children: ($$renderer4) => {
            ...
            $$renderer3.push(`<!----></div></div> <div class="flex justify-end items-center gap-1.5 shrink-0">`);
            if (accessRoles.includes("write")) {
              $$renderer3.push("<!--[0-->");
              $$renderer3.select(
                {
        ...
        if (getVisibility(accessGrants ?? []) === "private" && accessGroups.length === 0 && selectedUsers.length === 0) {
          $$renderer3.push("<!--[0-->");
          $$renderer3.push(`<div class="text-xs text-gray-500 text-center py-3">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("No access grants. Private to you."))}</div>`);
        } else {
          $$renderer3.push("<!--[-1-->");
        }
        $$renderer3.push(`<!--]--></div>`);
