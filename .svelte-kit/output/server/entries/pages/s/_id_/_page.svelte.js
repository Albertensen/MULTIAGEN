...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    const loadSharedChat = async () => {
      ...
      const shareId = store_get($$store_subs ??= {}, "$page", page).params.id;
      if (!shareId) return null;
      const userSettings = token ? await getUserSettings(token).catch((error) => {
      ...
      await models.set(token ? await getModels(token, store_get($$store_subs ??= {}, "$config", config)?.features?.enable_direct_connections ? store_get($$store_subs ??= {}, "$settings", settings)?.directConnections ?? null : null).catch((error) => {
        /* @__PURE__ */ console.error(error);
        return [];
      }) : []);
      ...
      if (chat) {
        user$1 = token ? await getUserInfoById(token, chat.user_id).catch((error) => {
          /* @__PURE__ */ console.error(error);
          return null;
        }) : null;
    ...
    canClone = store_get($$store_subs ??= {}, "$sessionUser", user) && (store_get($$store_subs ??= {}, "$sessionUser", user).role === "admin" || (store_get($$store_subs ??= {}, "$sessionUser", user).permissions?.chat?.import ?? true));
    ...
    function $$render_inner($$renderer3) {
      ...
      if (loaded) {
        ...
        if (canClone) {
          $$renderer3.push("<!--[0-->");
          $$renderer3.push(`<div class="absolute bottom-0 right-0 left-0 flex justify-center w-full bg-linear-to-b from-transparent to-white dark:to-gray-900"><div class="pb-5"><button class="px-3.5 py-1.5 text-sm font-normal bg-black hover:bg-gray-900 text-white dark:bg-white dark:text-black dark:hover:bg-gray-100 transition rounded-full">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Clone Chat"))}</button></div></div>`);
        } else {
          $$renderer3.push("<!--[-1-->");
        }
