  ...
  $$renderer.component(($$renderer2) => {
    ...
    if (sourceImages.length > 0) {
      ...
      for (let index = 0, $$length = each_array_1.length; index < $$length; index++) {
        ...
        $$renderer2.push(`<div class="relative group"><div class="relative flex items-center"><img${attr("src", image)} alt="" class="size-10 rounded-xl object-cover"/></div> <div class="absolute -top-1 -right-1"><button class="bg-white text-black border border-white rounded-full group-hover:visible invisible transition" type="button"${attr("aria-label", store_get($$store_subs ??= {}, "$i18n", i18n).t("Remove image"))}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" class="size-4"><path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z"></path></svg></button></div></div>`);
      }
      $$renderer2.push(`<!--]--></div>`);
    } else {
      $$renderer2.push("<!--[-1-->");
    ...
    $$renderer2.push(`</textarea></div> <div class="flex justify-between items-center gap-2 mt-2"><div class="shrink-0"><input type="file" accept="image/*" multiple="" class="hidden"/> <button type="button" class="px-3.5 py-1.5 text-sm font-normal bg-gray-50 hover:bg-gray-100 text-gray-900 dark:bg-gray-850 dark:hover:bg-gray-800 dark:text-gray-200 transition rounded-lg">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Add Image"))}</button></div> <div class="flex gap-2 shrink-0">`);
    {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<button${attr("disabled", prompt.trim() === "", true)} class="px-3.5 py-1.5 text-sm font-normal bg-black hover:bg-gray-900 text-white dark:bg-white dark:text-black dark:hover:bg-gray-100 transition rounded-lg disabled:opacity-50 disabled:cursor-not-allowed">${escape_html(store_get($$store_subs ??= {}, "$i18n", i18n).t("Run"))}</button>`);
    }
