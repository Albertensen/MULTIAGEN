...
//   3. WebSocket listener untuk event agen dari backend.
//   4. Murni frontend, tanpa backend — bisa diuji langsung di /utest.
// =====================================================================

// ==================== WebSocket Status Store ====================
export const wsStatus = writable<'connecting' | 'connected' | 'disconnected'>('connecting');
...
export const connectWebSocket = () => {
	...
	// Use ws:// if not secure, wss:// if HTTPS. Assuming localhost dev uses ws.
	...
	ws.onmessage = (evt) => {
		...
		if (type === 'agent:created') {
			assignAgent(payload.id);
		} else if (type === 'agent:updated') {
			assignAgent(payload.id);
		} else if (type === 'agent:deleted') {
			// Optionally emit an event for deletion
			emit('orchestration:agent-deleted', { chatId: '', fromAgentId: '', targetAgentId: payload.id, ts: Date.now() });
		}
	};
};
